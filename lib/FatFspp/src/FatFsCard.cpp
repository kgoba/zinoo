...
/* ===========================================================
                  ...
                  FatFsCard member functions

   =========================================================== */

bool FatFsCard::begin( uint8_t chipSelectPin, uint8_t sckDivisor )
{
...
uint8_t FatFsCard::cardCommand( uint8_t cmd, uint32_t arg )
 {
  ...
  // wait for response
  for( uint8_t i = 0;
       (( m_status = spiReceive()) & 0X80 ) && i != 0XFF;
       i++) ;

  return m_status;
}

void FatFsCard::chipDeselect()
{
  digitalWrite( m_CSPin, HIGH );
...
bool FatFsCard::readBlocks( uint32_t block, uint8_t* dst, size_t count )
{
  if( ! readStart( block ))
    return false;
...
bool FatFsCard::readData( uint8_t* dst, size_t count )
{
#if USE_SD_CRC
  uint16_t crc;
#endif  // USE_SD_CRC
...
bool FatFsCard::waitNotBusy( uint16_t timeoutMillis )
{
  uint16_t t0 = millis();
...
bool FatFsCard::writeBlocks( uint32_t block, const uint8_t* src, size_t count)
{
...
bool FatFsCard::writeData( uint8_t token, const uint8_t* src )
{
#if USE_SD_CRC
...
bool FatFsCard::writeStart( uint32_t blockNumber, uint32_t eraseCount )
{
